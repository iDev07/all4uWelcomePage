import React from "react";
import CheckCircleOutlinedIcon from "@mui/icons-material/CheckCircleOutlined";
import Fade from "react-reveal";
function Retail() {
  return (
    <div className="Retail">
      <div className="containerdev">
        <div className="intro">
          <h1>Розничный маркетплейс</h1>
          <p>500 000 + товаровов для вас и вашего дома, офиса</p>
        </div>
        <div className="retail_products">
          <div className="retail_prod_wrapper">
            <Fade left>
              <div className="product_wrap">
                <a href="">
                  <div className="prod_image">
                    <img src="https://all4u.market/images/thumbnails/550/450/detailed/26/44_ije7-ds.jpg.jpg" />
                  </div>
                  <div className="prod_status">
                    <div className="top_wrap">
                      <p>
                        <CheckCircleOutlinedIcon /> В наличии
                      </p>
                      <p>Узбекистан</p>
                    </div>
                  </div>
                  <div className="prod_price">
                    <div>
                      <h2>
                        13 111 111 <span>UZS</span>
                      </h2>
                      <h4>
                        {" "}
                        <s>13 111 111</s>
                        <span>UZS</span>
                      </h4>
                    </div>
                    <div className="discount">2%</div>
                  </div>
                  <div className="prod_about">
                    <div>
                      <h4>Samsung S22 Ultra 256GB Black </h4>
                      <p>Bsxzxbzsxmzbsnxm bzmxzsnbxm zbsAXAXAXXA cawawcx </p>
                    </div>
                  </div>
                </a>
              </div>
            </Fade>
            <Fade left>
              <div className="product_wrap">
                <a href="">
                  <div className="prod_image">
                    <img src="https://all4u.market/images/thumbnails/550/450/detailed/26/44_ije7-ds.jpg.jpg" />
                  </div>
                  <div className="prod_status">
                    <div className="top_wrap">
                      <p>
                        <CheckCircleOutlinedIcon /> В наличии
                      </p>
                      <p>Узбекистан</p>
                    </div>
                  </div>
                  <div className="prod_price">
                    <div>
                      <h2>
                        13 111 111 <span>UZS</span>
                      </h2>
                      <h4>
                        {" "}
                        <s>13 111 111</s>
                        <span>UZS</span>
                      </h4>
                    </div>
                    <div className="discount">2%</div>
                  </div>
                  <div className="prod_about">
                    <div>
                      <h4>Samsung S22 Ultra 256GB Black </h4>
                      <p>Bsxzxbzsxmzbsnxm bzmxzsnbxm zbsAXAXAXXA cawawcx </p>
                    </div>
                  </div>
                </a>
              </div>
            </Fade>
            <Fade left>
              <div className="product_wrap">
                <a href="">
                  <div className="prod_image">
                    <img src="https://all4u.market/images/thumbnails/550/450/detailed/26/44_ije7-ds.jpg.jpg" />
                  </div>
                  <div className="prod_status">
                    <div className="top_wrap">
                      <p>
                        <CheckCircleOutlinedIcon /> В наличии
                      </p>
                      <p>Узбекистан</p>
                    </div>
                  </div>
                  <div className="prod_price">
                    <div>
                      <h2>
                        13 111 111 <span>UZS</span>
                      </h2>
                      <h4>
                        {" "}
                        <s>13 111 111</s>
                        <span>UZS</span>
                      </h4>
                    </div>
                    <div className="discount">2%</div>
                  </div>
                  <div className="prod_about">
                    <div>
                      <h4>Samsung S22 Ultra 256GB Black </h4>
                      <p>Bsxzxbzsxmzbsnxm bzmxzsnbxm zbsAXAXAXXA cawawcx </p>
                    </div>
                  </div>
                </a>
              </div>
            </Fade>
            <Fade left>
              <div className="product_wrap">
                <a href="">
                  <div className="prod_image">
                    <img src="https://all4u.market/images/thumbnails/550/450/detailed/26/44_ije7-ds.jpg.jpg" />
                  </div>
                  <div className="prod_status">
                    <div className="top_wrap">
                      <p>
                        <CheckCircleOutlinedIcon /> В наличии
                      </p>
                      <p>Узбекистан</p>
                    </div>
                  </div>
                  <div className="prod_price">
                    <div>
                      <h2>
                        13 111 111 <span>UZS</span>
                      </h2>
                      <h4>
                        {" "}
                        <s>13 111 111</s>
                        <span>UZS</span>
                      </h4>
                    </div>
                    <div className="discount">2%</div>
                  </div>
                  <div className="prod_about">
                    <div>
                      <h4>Samsung S22 Ultra 256GB Black </h4>
                      <p>Bsxzxbzsxmzbsnxm bzmxzsnbxm zbsAXAXAXXA cawawcx </p>
                    </div>
                  </div>
                </a>
              </div>
            </Fade>
          </div>
          <a href="" className="click_retail product_wrap">
            <div className="for_back_wholesale">
              <div className="content__this">
                <h2>Посмотреть розничную торговую площадку</h2>
              </div>
            </div>
          </a>
        </div>
      </div>
    </div>
  );
}

export default Retail;
